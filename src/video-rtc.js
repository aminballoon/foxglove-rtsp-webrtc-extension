...
export class VideoRTC extends HTMLElement {
    ...
    onopen() {
        ...
        this.ws.addEventListener('message', ev => {
            if (typeof ev.data === 'string') {
                ...
                if (this.onlog) {
                    this.onlog(`[WS] ${msg.type}${msg.value ? ': ' + (typeof msg.value === 'string' && msg.value.length > 80 ? msg.value.substring(0, 80) + '...' : JSON.stringify(msg.value)) : ''}`);
                }
    ...
    onwebrtc() {
        ...
        pc.addEventListener('icecandidate', ev => {
            ...
            if (this.onlog && candidate) {
                this.onlog(`[ICE-local] ${candidate.trim()}`);
            }
        ...
        pc.addEventListener('connectionstatechange', () => {
            if (this.onlog) {
                this.onlog(`[PeerConnection] state: ${pc.connectionState}`);
            }
            if (pc.connectionState === 'connected') {
                const transceivers = pc.getTransceivers().filter(tr => tr.currentDirection === 'recvonly');
                transceivers.forEach(tr => {
                    if (tr.receiver) {
                        try {
                            if ('jitterBufferTarget' in tr.receiver) tr.receiver.jitterBufferTarget = 0;
                            if ('playoutDelayHint' in tr.receiver) tr.receiver.playoutDelayHint = 0;
                        } catch {}
                    }
                });
                const tracks = transceivers.map(tr => tr.receiver.track);
                /** @type {HTMLVideoElement} */
